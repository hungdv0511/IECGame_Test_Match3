...
public class Utils
{
    ...
    public static int GetTotalDefinedState<T>()
    {
        return Enum.GetNames(typeof(T)).Length;
    }


    public static T GetRandomDefinedState<T>(bool condition) where T : Enum
    {
        object value = URandom.Range(0, GetTotalDefinedState<T>());
        while (condition)
        {
            value = URandom.Range(0, GetTotalDefinedState<T>());
        }
        return (T)value;
    }
}
