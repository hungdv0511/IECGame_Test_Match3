...
public class Board
{
    ...
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                //for (int i = 0; i < Utils.GetTotalDefinedState<NormalItem.eNormalType>(); i++)
                //{
                //    eNormalTypes.Add((NormalItem.eNormalType)i);
                //}
                //NormalItem.eNormalType newType = GetLeastItemAmount(eNormalTypes, cell);
    ...
    private bool IsSameTypeWithNeighbour(Cell currentCell, Cell neighBourCell)
    {
        return currentCell.IsSameType(neighBourCell);
    }

    private bool IsNeighborCellHasSameType(Cell currentCell)
    {
        if (currentCell.NeighbourBottom != null)
        {
            if (currentCell.IsSameType(currentCell.NeighbourBottom))
            {
                return true;
            }
        }
        if (currentCell.NeighbourUp != null)
        {
            if (currentCell.IsSameType(currentCell.NeighbourUp))
            {
                return true;
            }
        }
        if (currentCell.NeighbourLeft != null)
        {
            if (currentCell.IsSameType(currentCell.NeighbourLeft))
            {
                return true;
            }
        }
        ...
        {
            if (currentCell.IsSameType(currentCell.NeighbourRight))
            {
                return true;
            }
        }
    ...
    internal void ShiftDownItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            ...
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                holder.Assign(item);
                item.View.DOMove(holder.transform.position, 0.3f).SetEase(Ease.OutElastic);
            }
        }
    }
    ...
    private NormalItem.eNormalType GetLeastItemAmount(List<NormalItem.eNormalType> eNormalTypes, Cell currentCell)
    {
        if (currentCell.NeighbourBottom != null)
        {
            NormalItem bottomNeighbourItem = currentCell.NeighbourBottom.Item as NormalItem;
            if (bottomNeighbourItem != null)
            {
                eNormalTypes.Remove(bottomNeighbourItem.ItemType);
            }
        }
        if (currentCell.NeighbourUp != null)
        {
            NormalItem upNeighbourItem = currentCell.NeighbourUp.Item as NormalItem;
            if (upNeighbourItem != null)
            {
                eNormalTypes.Remove(upNeighbourItem.ItemType);
            }
        }
        if (currentCell.NeighbourLeft != null)
        {
            NormalItem leftNeighbourItem = currentCell.NeighbourLeft.Item as NormalItem;
            if (leftNeighbourItem != null)
            {
                eNormalTypes.Remove(leftNeighbourItem.ItemType);
            }
        }
        if (currentCell.NeighbourRight != null)
        {
            NormalItem rightNeighbourItem = currentCell.NeighbourRight.Item as NormalItem;
            if (rightNeighbourItem != null)
            {
                eNormalTypes.Remove(rightNeighbourItem.ItemType);
            }
        }
        ...
        for (int i = 0; i < eNormalTypes.Count; i++)
        {
            ...
            int countItem = CountItemByType(type);
            if (minAmount >= countItem && countItem > 0)
            {
                minAmount = countItem;
                result = type;
            }
    ...
    private int CountItemByType(NormalItem.eNormalType eNormalType)
    {
        int countItem = 0;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if (m_cells[x, y] != null && m_cells[x, y].Item is NormalItem)
                {
                    NormalItem normalItem = m_cells[x, y].Item as NormalItem;
                    if (normalItem.ItemType == eNormalType && normalItem != null)
                    {
                        countItem++;
                    }
                }
